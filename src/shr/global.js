...
var uriex = 'data:application/vnd.ms-excel;base64,',
    templateex = '<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns="http://www.w3.org/TR/REC-html40"><head><!--[if gte mso 9]><xml><x:ExcelWorkbook><x:ExcelWorksheets><x:ExcelWorksheet><x:Name>{worksheet}</x:Name><x:WorksheetOptions><x:DisplayGridlines/></x:WorksheetOptions></x:ExcelWorksheet></x:ExcelWorksheets></x:ExcelWorkbook></xml><![endif]--></head><body><table>{table}</table></body></html>',
    base64ex = function (s) {
...
var
    ...
    _LZString = function (sKey) {
        ...
        var //f = String.fromCharCode,
            baseReverseDic = {},
            //_mathPow = Math.pow,
            _key = angular.isUndefined(sKey) ? _genKey() : angular.isNumber(sKey) ? _genKey(sKey) : sKey,
        ...
        return {
            toBase64: function (uncompressed) { //toBase64
                if (isNull(uncompressed)) return "";
                //return _checkPad(_compress(uncompressed, 6, function(a) { return _key.charAt(a); }), 4);
                return _checkPad(_compress(uncompressed, 6, function (a) {
            ...
            fromBase64: function (compressed) { //fromBase64
            ...
            toB128: function (uncompressed) { //toB128
                if (isNull(uncompressed)) return "";
                //return _checkPad(_compress(uncompressed, 7, function(a) { return _key.charAt(a); }), 8);
                return _checkPad(_compress(uncompressed, 7, function (a) {
                    return charAt(_key, a);
                }), 8);
            },

            fromB128: function (compressed) { //fromB128
                if (isNull(compressed)) return "";
                if (compressed == "") return null;
            ...
            toUTF16: function (uncompressed) { //toUTF16
                if (isNull(uncompressed)) return "";
                return _compress(uncompressed, 15, function (a) {
                    return StringFromCharCode(a + 32);
                }) + " "; //f(a + 32); }) + " ";
            },

            fromUTF16: function (compressed) { //fromUTF16
                if (isNull(compressed)) return "";
                if (compressed == "") return null;
            ...
            compress: function (uncompressed) { //compress
                return _compress(uncompressed, 16, function (a) {
                    return StringFromCharCode(a);
                }); //f(a); });
            },

            decompress: function (compressed) { //decompress
...
return {
    ...
    value: function (key, value) {
        if (angular.isDefined(value)) { //setValue
            _Value[key] = value;
        } else {
     ...
     */
    isValExist: function (key) { //isValueExist
        if (_Value[key]) {
            return CONST_TRUE;
        } else {
            return CONST_FALSE;
    ...
    dec: function (source, edType, nBitLen) {
        if (angular.isUndefined(edType)) { //default base 128 decrypt
            return _nBitDec(source);
        } else { //base 64 uri safe decrypt
            return _nBitDec(source, nBitLen || 6, angular.isNumber(edType) ? _genKey(edType) : edType);
        }
    },
    ...
    enc: function (source, edType, nBitLen) {
        if (angular.isUndefined(edType)) { //default base 128 encrypt
            return _nBitEnc(source);
        } else { //base 64 uri safe encrypt
            return _nBitEnc(source, nBitLen || 6, angular.isNumber(edType) ? _genKey(edType) : edType);
        }
    },
